...
test("×とキャンセルは送信ではなく閉じる専用ボタンになっている", () => {
  ...
  assert.deepEqual(closeButtons.map((match) => match[1]).sort(), ["csv-import-dialog", "csv-import-dialog", "filter-dialog", "filter-dialog", "project-dialog", "project-dialog", "task-dialog", "task-dialog"]);
...
test("表示形式を切り替えると選択したビュー以外は確実に隠れる", () => {
  assert.match(script, /elements\.list\.hidden = upcomingView \|\| \(trashView \? false : state\.mode !== "list"\)/);
...
test("近日予定は表示切替のない専用週間カレンダーになる", () => {
  ...
  assert.match(script, /elements\.viewSwitcher\.hidden = trashView \|\| upcomingView/);
